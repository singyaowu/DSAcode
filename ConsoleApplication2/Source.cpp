...
void saveDictionary(char dictionary[], const char* address, const int n,/* vector <char*> &Nphrase,vector<long> &frequency*/vector<Data> &NphraseFre,TriesNode &tree)

{
	...
	//char frequencyString[15];//既frequency
	int indexSpace = 0;
...
{	
	...
	if ((questionMarkPostion = query.find('?')) != -1) {
		...
		copiedQuery.erase(questionMarkPostion, sapcePosition - questionMarkPostion + 1);//+1琌痙" "
		expandQuery(copiedQuery, expandedQueries);
	}
	//find "/"
	...
	//find "*"
		...
		for (int i = 0; i < length; i++) {//计单フ计
			if (copiedQuery[i] == ' ') {
				numWord++;
			}
		...
		numStar--;//搭瞷矪瞶奔*
		numWord = 5 - numWord + numStar; //础" _ "计ヘ5 - (numWord - numStar)
		copiedQuery.erase(starPosition, 2);//抖獽奔" "
		if (numWord < 4) {//碞琌Τ ぃ才砏玥 ﹚璶Τぶㄢ
			expandQuery(copiedQuery, expandedQueries);
		}
		for (int i = 0; i < numWord; i++) {
			copiedQuery.insert(starPosition, "_ ");//ぶ"_ "
			expandQuery(copiedQuery, expandedQueries);
		}
	}
	else {
...
void searchQuery(const vector<string> &expandedQueries, TriesNode N[], vector <int> indexMatchForLines[])
{
	...
	for (int numExpTest = 0; numExpTest < numExpandedQueries; numExpTest++) {
		...
		else if (indexMaxList == 0 && listToIntersection[0] != NULL) {//Τ
...
#pragma region  Save
	...
	/*saveDictionary(dictionary[0], path2.c_str(), Nphrase[0], 2, frequency[0], N[0]);
	saveDictionary(dictionary[1], path3.c_str(), Nphrase[1], 3, frequency[1], N[1]);
	saveDictionary(dictionary[2], path4.c_str(), Nphrase[2], 4, frequency[2], N[2]);
	saveDictionary(dictionary[3], path5.c_str(), Nphrase[3], 5, frequency[3], N[3]);*/
	saveDictionary(dictionary[0], path2.c_str(), 2, NphraseFre[0], N[0]);
	saveDictionary(dictionary[1], path3.c_str(), 3, NphraseFre[1], N[1]);
	saveDictionary(dictionary[2], path4.c_str(), 4, NphraseFre[2], N[2]);
	saveDictionary(dictionary[3], path5.c_str(), 5, NphraseFre[3], N[3]);
#pragma endregion
	//input queries
	string query;
	...
	while (getline(cin, query))
	{
		...
		searchQuery(expandedQueries, N, indexMatchForLines);
		//compute frequency
		long temp = 0;
		...
		for (int i = 0; i < 4; i++) {
			...
			for (it = indexMatchForLines[i].begin(); it != indexMatchForLines[i].end(); ++it) {///////////////////**/+++++++++++
				indexSatis.push_back(NphraseFre[i][*it]);
